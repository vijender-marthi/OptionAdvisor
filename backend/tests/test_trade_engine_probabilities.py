...
class TradeEngineProbabilityMathTest(unittest.TestCase):
    def test_long_call_pop_and_ev_use_breakeven_and_directional_drift(self):
        ...
        expected_ev = round(_expected_option_payoff(PRICE, leg.strike, leg.iv, expiry, "CALL", drift) - leg.mid_price, 4)
    ...
    def test_bull_put_spread_uses_full_payoff_distribution(self):
        ...
        expected_prob_max_loss = round(1.0 - _prob_above(PRICE, buy_leg.strike, avg_iv, expiry, drift), 4)
        expected_ev = round(
            trade["net_credit"]
